...
params.mean_scaling=3*(0:C-1); %Meams [0 3 6 9]
params.Cov_scaling=6*ones(1,C);%1:C; %Covariances [6 6 6 6]
gmmParameters.priors = [0.3 0.3 0.4];% % priors 
...
figure;
for ind=1:C
    plot(x(1,labels==ind),x(2,labels==ind),symbols(ind),'DisplayName',['Class ' num2str(ind)]);
hold on;
end
xlabel('x1');
ylabel('x2');
...
for ind3=1:length(lossMatrix) 
    ...
    for ind=1:C
        ...
        plot(x(1,class_ind & ~fDecision_ind.(lossMatrix{ind3})),x(2,class_ind & ~fDecision_ind.(lossMatrix{ind3})),symbols(ind),'Color',[0.39 0.83 0.07],'DisplayName',['Class ' num2str(ind) ' Correct Classification']);
        hold on;
        plot(x(1,class_ind & fDecision_ind.(lossMatrix{ind3})),x(2,class_ind & fDecision_ind.(lossMatrix{ind3})),['r' symbols(ind)],'DisplayName',['Class ' num2str(ind) ' Incorrect Classification']);
        hold on; 
    end
    ...
    for ind2=1:C
        ...
        plot(x(1,fDecision_ind.(lossMatrix{ind3}) & labels==ind2),x(2,fDecision_ind.(lossMatrix{ind3}) & labels==ind2),'g.','DisplayName','Incorrectly Classified'); ylabel('x2');
        grid on;
