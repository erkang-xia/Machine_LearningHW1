...
%Homework #1
%Problem #2
%Significant parts of this code were derived from the following sources %g/practice/ERMwithClabels.m
%g/practice/evalGaussian.m
%g/practice/generateDataFromGMM.m %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %Expected risk minimization with 2 classes
clear all, close all,
C = 3; % 3 label
N = 10000;
...
params.mean_scaling=8*(0:4-1); %Meams [0 3 6 9]
params.Cov_scaling=6*ones(1,4 );%1:C; %Covariances [6 6 6 6]
gmmParameters.priors = [0.3 0.3 0.4 ];%[0.2 0.25 0.25 0.3]; % priors 
gmmParameters.meanVectors(1,:) = params.mean_scaling; % arbitrary mean vectors
%[0,3,6,9/0,0,0,0]
gmmParameters.meanVectors(2,:) = zeros(1,4); % arbitrary mean vectors
gmmParameters.meanVectors(3,:) = zeros(1,4); % arbitrary mean vectors


%Define loss matrices
lossMatrix={'minErr' 'deltaU'};
lossMatrixA.minErr = ones(C,C)-eye(C); % For min-Perror design, use 0-1 loss
lossMatrixA.deltaU= [0 1 100 ;
    1 0 100 ;
    1 1 0];
for ind = 1:C
...
for ind = 1:C
    pxgivenl(ind,:) = evalGaussian(x,gmmParameters.meanVectors(:,ind),gmmParameters.covMatrices(:,:,ind)); % Evaluate p(x|L=l)
end
...
for ind3=1:length(lossMatrix) 
    ...
    %Expected Risk
    ExpRisk.(lossMatrix{ind3})= gmmParameters.priors*diag(expectedRisksA.(lossMatrix{ind3})'*confMatrix.(lossMatrix{ind3}));
    ...
    plot3(x(1,class_ind),x(2,class_ind),x(3,class_ind), symbols(ind),'DisplayName',['Class ' num2str(ind)]);
    hold on;
    end
    xlabel('x1');
    ylabel('x2');
    ...
    grid on;
    title('X Vector with Classified Values'); legend 'show';
    %Plot Decisions with Incorrect Results as specified in assignment
    figure;
    for ind=1:C
        class_ind=decisionsA.(lossMatrix{ind3})==ind; 
        plot3(x(1,class_ind & ~fDecision_ind.(lossMatrix{ind3})),x(2,class_ind & ~fDecision_ind.(lossMatrix{ind3})),x(3,class_ind & ~fDecision_ind.(lossMatrix{ind3})),symbols(ind),'Color',[0.39 0.83 0.07],'DisplayName',['Class ' num2str(ind) ' Correct Classification']);
        hold on;
        plot3(x(1,class_ind & fDecision_ind.(lossMatrix{ind3})),x(2,class_ind & fDecision_ind.(lossMatrix{ind3})),x(3,class_ind & fDecision_ind.(lossMatrix{ind3})),['r' symbols(ind)],'DisplayName',['Class ' num2str(ind) ' Incorrect Classification']);
        hold on; 
    end
    ...
    for ind2=1:C
        subplot(4,1,ind2);
        for ind=1:C
            class_ind=decisionsA.(lossMatrix{ind3})==ind; plot(x(1,class_ind),x(2,class_ind),symbols(ind),'DisplayName',['Class ' num2str(ind)]);
            hold on;
        end
        plot3(x(1,fDecision_ind.(lossMatrix{ind3}) & labels==ind2),x(2,fDecision_ind.(lossMatrix{ind3}) & labels==ind2),x(3,fDecision_ind.(lossMatrix{ind3}) & labels==ind2),'g.','DisplayName','Incorrectly Classified'); ylabel('x2');
        grid on;
...
%% ======================= Question 2: Functions ====================== %% %From g/Code/evalGaussian.m %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
C = 4; % Number of components
x = zeros(n,N); labels = zeros(1,N); %n = 3
% Decide randomly which samples will come from each component
u = rand(1,N); thresholds = [0.3, 0.6,0.8,1];
for l = 1:C
    if l <=2
        ...
        labels(1,indl) = l*ones(1,Nl)
        u(1,indl) = 1.1*ones(1,Nl); % these samples should not be used again 
        x(:,indl) = mvnrnd(meanVectors(:,3),covMatrices(:,:,3),Nl)';
    end
